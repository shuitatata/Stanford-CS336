...
def update_cfg_from_wandb(cfg, wandb_config):
    for key, value in wandb_config.items():
        if "." in key:
            ...
            for k in keys[:-1]:
                if k not in curr:
                    curr[k] = {}
                curr = curr[k] = curr[k] if k in curr else {}  # 简单的防错
            curr[keys[-1]] = value
        else:
            cfg[key] = value
...
def main(cfg):
    ...
    wandb.init(
        project=wandb_cfg.get("project", "cs336-assignment1-sweep"),
        config=cfg,  # 传入默认配置
    )

    if wandb.config:
        update_cfg_from_wandb(cfg, wandb.config)
        # 重新获取配置引用，确保生效
        training_cfg = cfg["training"]
        optimizer_cfg = cfg["optimizer"]
        model_cfg = cfg["model"]
...
# CUDA_VISIBLE_DEVICES=3 wandb agent tz-zhang/Stanford-CS336-assignment1-basics/655wl2pu
# wandb sweep --stop tz-zhang/Stanford-CS336-assignment1-basics/lqxdks1m